...
| D41 | The npm publish workflow's `smoke-test` job installs the just-published tarball into an isolated temp project and exercises its real exports, rather than re-running the local test suite or a full `materialize()` against real mise/network | The existing `test` and `mutation-test` CI jobs both run against `dist/` directly (via relative imports in the test files) -- neither one ever goes through what `npm install` alone actually resolves, so a packaging mistake (a wrong `files`/`main`/`types` entry in `package.json`, an export accidentally left out of `src/index.ts`) could pass every existing check and still leave a broken published package, undetected until a real consumer's own `npm install` failed. Deliberately not a full `materialize()` run against real mise/network: that path is already covered thoroughly by `test/convergence.test.ts` against the local build (D-see the coverage/mutation entries) -- re-proving the same resolution logic works, now against a published copy, tests packaging correctness at best and mostly just re-pays the same ~50s cold-mise-install cost this repo already goes out of its way to avoid paying twice (D-see the mise-install-lock comments). The exports it does check (`validateManifest`, `Materializer`, `MiseEnvironmentProvider`, `OfficialMcpRegistry`, `writeLockfile`) are exactly the public API surface documented in `README.md`'s own Usage section -- the smoke test's real job is confirming that documented surface actually exists and is callable on what a consumer just installed. Retries `npm install` up to 5 times with a 15s backoff before failing for real: npm registry propagation after a fresh publish is not always instant, and a job that fails on lag rather than an actual problem would need a manual re-run every time, defeating the point of an automated gate. |
| D42 | The CLI's shared logic (#94) lives in `src/cli.ts`, compiled by `tsc` into the npm package's `bin` entry (`dist/cli.js`); `bin/ribosome.ts`, the `bun build --compile` target for the standalone-binary track, delegates to it (`import "../src/cli"`) rather than duplicating it | Before this, `bin/ribosome.ts` held the only CLI implementation and was a `bun`-only target (`#!/usr/bin/env bun`, imports straight from `../src/**`) -- fine for the compiled binary, but unusable as an npm `bin` entry, since `tsc`'s build is deliberately scoped to `rootDir: "src"` (D30) and can't emit a file living outside it. The alternative -- copy-pasting the CLI logic into a second file under `src/` -- was rejected outright: two ~180-line implementations of the same `resolve`/`prune` argument parsing and dispatch would inevitably drift, and the whole point of shipping a second entry point is that both need to behave identically. `src/cli.ts` needed no logic changes to move, only import paths (`./adapters/...` instead of `../src/adapters/...`) and the shebang (`node`, not `bun`) -- it was already plain Node-API code (`node:fs/promises`, `node:path`), nothing Bun-specific beyond the shebang itself. Classified as `"other"` by the architecture fitness function's `classify()` (same bucket as `index.ts`, not `"orchestrator"`), so it's free to import `adapters/`/`ports`/`orchestrator` directly like `index.ts` already does -- it's a driving adapter, not part of the orchestrator layer the fitness function's rules constrain. `dist/cli.js` gets its executable bit via a `postbuild` script (`chmod +x`, the same npm-lifecycle-hook pattern `ribosome-schema`'s `bindings/typescript` already uses for its own postbuild step) rather than relying solely on npm's own bin-linking to set it, since the file needs to be executable when run directly too (`./dist/cli.js`), not just via the `node_modules/.bin` symlink npm creates. Neither `src/cli.ts` nor `bin/ribosome.ts` (exercised only via child-process spawn in `test/cli.test.ts` and `test/cli-node.test.ts`, same as before) shows up in `bun test`'s in-process coverage report, so D37's per-file floor is unaffected. |
| D43 | D16 implemented as designed, in one file: `resolveMiseBinary()` (`src/vendor/resolve-mise-binary.ts`) resolves `RIBOSOME_MISE_BIN` env var > a `mise-bundled` sibling directory next to the running executable > bare `mise` on `PATH`; `MiseEnvironmentProvider` resolves it once per instance instead of hardcoding `"mise"` | The bundled-path tier needed a concrete, verifiable "where does a packaged binary's sibling mise actually live" answer, not just the design's "e.g. `/usr/lib/ribosome/mise`" sketch -- verified empirically (a real `bun build --compile` binary, run through a symlink and from another cwd) that `process.execPath` resolves to the compiled binary's own real path, never bun's own path, which a plain `node`-run `dist/cli.js` does NOT get (there, `process.execPath` is node's own binary) -- so `dirname(process.execPath)/mise-bundled/<bin>` is a convention that self-selects: it only ever resolves to something real inside a packaged binary artifact, and harmlessly never matches for `npm install`-as-a-library consumption, with no runtime-detection branch needed to keep that guarantee. `resolveMiseBinary()` takes its env/execPath/platform/exists as injectable options (all defaulting to the real `process.*`/`node:fs`) so its three tiers are unit-tested directly (`test/resolve-mise-binary.test.ts`) without needing a real compiled binary or a real bundled mise on disk. Checksums+fetching are a separate concern from resolution: `src/vendor/mise-version.ts` (the pin, generated by `node scripts/vendor-mise-update.js <version>`, never hand-edited) records only the five raw binaries' SHA-256s -- the actual multi-hundred-MB downloads happen at CI release-build time (`scripts/fetch-bundled-mise.js`, wired into `cross-compile.yml`, verified against those checksums before being uploaded as part of the same cross-compiled artifact bundle) and are never committed to the repo, unlike the small vendored MCP schema JSON in the sibling `ribosome-schema` repo. Drift detection (#9) mirrors that repo's `vendor-drift.yml` exactly in shape but checks two different things given there's no local file to hash: immutability (the pinned version's `SHASUMS256.txt` still matches the recorded checksums) and staleness (a newer mise release now exists) -- `scripts/check-mise-drift.js` + `.github/workflows/mise-drift.yml`, scheduled weekly, opening/updating a `vendor-drift`-labeled issue on either failure. The exact package-private final placement (`.deb`/`.rpm`'s own private lib dir vs. a zip/NSIS sibling file) is for the not-yet-built per-OS packaging jobs (#7/#10/#11) to decide -- their one hard constraint, set here, is that wherever they place the ribosome binary, `mise-bundled/<bin>` must sit directly beside it for `resolveMiseBinary()` to find it. |
| D44 | Per-OS packaging (#7/#10/#11) implemented as three `node` scripts (`scripts/package-{linux,macos,windows}.js`) invoked from a new `package.yml` workflow's per-target jobs, all still on `ubuntu-latest` -- Linux gets `.tar.gz` + `.deb` (`dpkg-deb`) + `.rpm` (`rpmbuild`, apt-installable and run natively for `x64`; run inside an emulated container for `arm64` -- see below), macOS gets a zip only (installer explicitly deferred, see D-see #17), Windows gets an NSIS installer (`makensis`, apt-installable) + a portable zip | Every format places the ribosome binary and a `mise-bundled/<bin>` sibling directory together, satisfying D43's one hard constraint: `.deb`/`.rpm` under a package-private `/usr/lib/ribosome/`, symlinked onto `PATH` from `/usr/bin/ribosome` (confirmed `process.execPath` resolves through a symlink to the real binary's own directory, so the symlink doesn't break tier-2 resolution); tar.gz/zip as a wrapping `ribosome-<os>-<arch>/` directory; the NSIS installer into `$PROGRAMFILES64\ribosome`. **`.rpm` needed absolute input paths, `.deb` didn't**: `rpmbuild`'s `%install` scriptlet runs from its own `_builddir`, not this script's `cwd`, so the binary/mise source paths written into the generated spec file are resolved via `node:path`'s `resolve()` before being embedded as shell text -- caught by staging both formats through the same local dry run and diffing the actual `%install` output, not by inspection alone. **`rpmbuild` refuses to build for `aarch64` on an x86_64 runner at all -- four real CI round trips before landing on why**, each disproving the last theory: `--target aarch64-linux` failed (`error: No compatible architectures found for build`); the spec's own `BuildArch: aarch64` alone, no `--target`, hit the *identical* error, disproving "`--target` is the trigger"; registering a QEMU binfmt handler (`docker/setup-qemu-action@v3`) so the kernel can transparently execute aarch64 code *still* hit the identical error, disproving "it's a runtime-execution-capability check" -- the compatible-architecture gate is a static table lookup (`rpmMachineScore` against `/usr/lib/rpm/rpmrc`), never a dynamic probe of what the host can actually run, so satisfying real execution capability was never going to help. The fix that actually works sidesteps the gate instead of trying to satisfy it: for the `arm64` leg only, `rpmbuild` itself runs as a genuinely emulated aarch64 *process* (`docker run --platform linux/arm64` against `rockylinux:9`, using that same registered QEMU binfmt handler, but now for something it's actually built for) inside a container, so `rpmbuild`'s own `uname`-equivalent reports aarch64 and the package is now the *native* case from its perspective -- the cross-arch gate is never reached because there's no cross-arch situation left by the time it checks. `x64` keeps building natively (already proven working; no reason to containerize it too). The container runs as root, so its output is `chown`-ed back to the CI user afterward -- otherwise the plain `copyFileSync` a few lines later, running as the unprivileged host process, can't read it back out. **Windows PATH integration avoids NSIS plugins entirely**: rather than vendor a third-party `.nsh` (e.g. the common `EnvVarUpdate.nsh`) or depend on a plugin the apt-packaged `nsis` might not ship, the installer's `nsExec::ExecToLog` shells out to PowerShell (built into every target Windows version) to read/write the machine `PATH` via `[Environment]::GetEnvironmentVariable`/`SetEnvironmentVariable` -- `nsExec` itself is part of NSIS's own base stub, not a separate plugin. Every packaging job builds, but none *installs and runs*, its artifact -- that's deliberately the separate verification-tier job (#13), which is the only place a `.deb`/`.rpm` actually gets `apt`/`dnf`-installed into a container matching its target distro family, and the only place the NSIS installer runs on real Windows. Local end-to-end validation was consequently partial: `dpkg-deb`, `tar`, and the staging logic were verified directly against a real build (`dpkg-deb -c`/`-I`, `tar tzvf`); `rpmbuild`, `makensis`, and `zip` are absent from the sandboxed dev environment this was built in, so those three legs were validated by dispatching the real `package.yml` workflow on GitHub Actions and inspecting the run -- which is exactly what caught both the `--target` bug above and a matching one in `package-macos.js` (`zip`'s output path was built relative to `outDir` while `zip` itself ran with `cwd: work`, resolving against the wrong directory; `package-windows.js`'s equivalent path was already wrapped in `resolve()`, which is what the fix mirrors). Matrix jobs (`package-linux`, `package-macos`) run with `fail-fast: false` precisely because of this: the first CI run had `linux-x64` and `darwin-arm64` both cancelled mid-run by their sibling architecture's failure, hiding whether they'd have passed on their own. |
- **Real, here:** the ports; the wiring that consumes `@medullaflow/ribosome-schema`
  as a published npm dependency, `^0.1.9` (verified by `test/schema-dependency.test.ts`);
