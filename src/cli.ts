...
// The CLI entry point (#5, #94): the seam between the pure orchestrator core
// (effects-free, testable against fakes -- see orchestrator/) and a
...
// aggregated-failure shape), and the exit-code contract test/cli.test.ts
// (via bin/ribosome.ts) and test/cli-node.test.ts (via this file's own
// tsc-compiled dist/cli.js) assert against.
//
// This is the npm package's `bin` entry (package.json's "bin" field points
// at dist/cli.js, this file's tsc output) -- what `npx @medullaflow/ribosome`
// and `npm install -g` actually run. bin/ribosome.ts, the separate
// `bun build --compile` target for the standalone binary track, delegates
// here rather than duplicating this logic (see that file's own comment).
...
import { LOCKFILE_FILENAME, writeLockfile } from "./orchestrator/lockfile-writer";
import { Materializer, ResolutionError } from "./orchestrator/materializer";
