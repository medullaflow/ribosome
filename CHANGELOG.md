...
- **Vendor a pinned mise binary, closing #8 and #9** — packaged binary
  artifacts will now bundle mise instead of requiring a pre-installed one.
  `MiseEnvironmentProvider` resolves its `mise` executable in order: an
  explicit `RIBOSOME_MISE_BIN` override, then a package-private
  `mise-bundled` sibling directory next to the running executable, then a
  bare `mise` on `PATH` (today's unmodified behavior — unaffected for
  `npm install`-as-a-library consumers, which ship no bundled binary). The
  pinned version + per-target SHA-256 checksums live in
  [`src/vendor/mise-version.ts`](src/vendor/mise-version.ts), generated by
  `node scripts/vendor-mise-update.js <version>`, never hand-edited.
  `.github/workflows/cross-compile.yml` now also fetches and
  checksum-verifies mise for all five release targets
  (`scripts/fetch-bundled-mise.js`), uploading them alongside the compiled
  `ribosome` binaries for a future packaging step to place side-by-side. A
  new scheduled workflow, `.github/workflows/mise-drift.yml`
